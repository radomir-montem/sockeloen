...
class VariantSelects extends HTMLElement {
  constructor() {
    ...
    this.variantData = JSON.parse(
      this.variantRadios.querySelector('script[type="application/json"]')
        .textContent
    )
    this.tags = JSON.parse(
      this.variantRadios.querySelector('script[type="application/json"].tags')
        .textContent
    )
  ...
  onVariantChange() {
    ...
    !this.isBundle && this.updateSizeOptions(this.variantData)
    this.checkDynamicCheckout()

    if (!this.currentVariant) {
      this.toggleAddButton(true, '', true)
      this.setUnavailable()
      this.setAvailableVariant()
    } else {
      this.filterMedia()
      this.updateMedia()
      this.updateURL()
  ...
  updateSizeOptions(variantsData) {
    ...
    let sizeContainer = document.querySelectorAll(
      '.js.product-form__input:not(.color-input)'
    )
    if (sizeContainer.length === 0) return

    let sizeInputs = Array.from(sizeContainer[0].childNodes).filter(
      (sizeInput) => sizeInput.nodeName.includes('INPUT')
    )
  ...
  filterMedia() {
    ...
    if (
      selected_variant.includes(selected_color) ||
      selected_variant.startsWith(selected_color)
    ) {
  ...
  renderProductInfo() {
    fetch(
      ...
      .then((responseText) => {
        ...
        const comingSoonProduct = html.querySelector(
  ...
  changeInventoryState() {
    ...
    salesPoints.forEach(salesPoint => {
      salesPoint.classList.add('hidden');
    });

    const salesPoint = document.querySelector(`.sales-point[data-variant-id="${this.currentVariant.id}"]`);
    if (salesPoint) {
      salesPoint.classList.remove('hidden');
    }
  }
}
